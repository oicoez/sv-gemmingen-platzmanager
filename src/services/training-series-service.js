...
export async function addTrainingSeries(input){
  ...
  const allowed=div===1?["exclusive","flexible"]:div===3?["one_third","two_thirds","exclusive"]:["flexible","half_a","half_b","exclusive"];
  if(!allowed.includes(allocationMode))throw new Error("Diese Belegung passt nicht zur Teilbarkeit des Trainingsortes");
  ...
  const address=def.location_address||"";
  ...
  if(!dates.length)throw new Error("Für diese Trainingsserie wurden keine Termine berechnet");
