...
document.querySelectorAll(".tab").forEach(b=>b.onclick=()=>{
 ...
 ["dashboard","week","month","training","games"].forEach(v=>$(`view-${v}`).classList.toggle("hidden",b.dataset.view!==v));
 if(b.dataset.view==="dashboard")loadDashboard();
 if(b.dataset.view==="week")loadWeek();
 if(b.dataset.view==="month")loadMonth();
 if(b.dataset.view==="training")loadTrainings();
});
...
async function loadTrainings(){
 try{
  ...
  $("trainingList").innerHTML=items.length?`<table><thead><tr><th>Datum</th><th>Zeit</th><th>Mannschaft</th><th>Ort</th><th>Platz</th><th>Modus</th><th>Konflikt</th><th></th></tr></thead><tbody>${items.map(x=>{
   const conflict=conflictById.get(x.id)||"";
   return `<tr class="${conflict?"conflictRow":""}"><td>${fmtDate(x.event_date)}</td><td>${String(x.start_time).slice(0,5)}–${String(x.end_time).slice(0,5)}</td><td>${esc(x.team)}</td><td>${esc(x.location)}</td><td>${esc(x.base_name)}</td><td>${esc(({flexible:"flexibel",half_a:"Hälfte A",half_b:"Hälfte B",exclusive:"Gesamt exklusiv"}[x.allocation_mode]||x.allocation_mode))}</td><td>${conflict?`<span class="conflictPill">Konflikt</span><br>${esc(conflict)}`:"–"}</td><td><button class="delTr" data-id="${x.id}" ${pin?"":"disabled"}>Löschen</button></td></tr>`;
...
$("saveTraining").onclick=async()=>{try{
 const body={teamId:$("trTeam").value,date:$("trDate").value,start:$("trStart").value,end:$("trEnd").value,locationId:$("trLocation").value,baseName:$("trBase").value,allocationMode:$("trMode").value,note:$("trNote").value};
 await json("/api/v5/trainings",{method:"POST",headers:{"content-type":"application/json","x-edit-pin":pin},body:JSON.stringify(body)});
 $("trainingMsg").textContent="Training gespeichert.";
