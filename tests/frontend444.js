...
const $=id=>document.getElementById(id);let pin="",timer=null,includePast=false,weekStart="",monthValue="",serverToday="",editingTrainingId="",resourceCache=[],trainingMode="single",editingGameId="";
...
$("login").onclick=async()=>{const p=prompt("Bearbeitungs-PIN:");if(!p)return;try{await json("/api/v5/auth/check",{method:"POST",headers:{"content-type":"application/json","x-edit-pin":p}});pin=p;$("sync").disabled=false;$("saveTraining").disabled=false;$("addTeam").disabled=false;$("saveGame").disabled=false;$("resetGames").disabled=false;$("login").textContent="Bearbeitungsmodus aktiv";loadTrainings()}catch(e){alert(e.message)}};
...
async function loadWeek(useServerCurrent=false){
 try{
  ...
  $("weekDays").innerHTML=j.days.map((d,idx)=>`<div class="day"><div class="dayhead">${dayNames[idx]}, ${fmtDate(d.date)}</div>${d.groups.length?d.groups.map(g=>`<div class="pitch"><div class="pitchtitle">${esc(g.location)} · ${esc(g.baseName)}</div>${g.segments.map(s=>`<div class="seg ${s.conflict?"conflict":(s.items.length===2?"split":"")}"><div class="time">${s.start}–${s.end}</div>${s.items.map(i=>`<div class="${i.eventType==="training"?"trainingClick":(i.eventType==="home_match"?"trainingClick":"")}" ${i.eventType==="training"?`data-training-id="${esc(i.id)}" title="Training bearbeiten"`:(i.eventType==="home_match"?`data-game-id="${esc(i.id)}" title="Spiel bearbeiten"`:"")}>${i.eventType==="home_match"?"⚽":"●"} ${esc(i.label)} · <span class="allocationPill ${esc(i.section)}">${esc(i.sectionLabel)}</span></div>`).join("")}${s.conflict?`<div class="conflictText">${esc(s.reason)}</div>`:""}</div>`).join("")}</div>`).join(""):`<div class="empty">frei</div>`}</div>`).join("");
 ...
 try{
  ...
  for(const d of j.days){
   ...
   for(const g of d.groups)for(const s of g.segments){
    ...
    entries.push(`<div class="monthitem ${s.conflict?"conflict":(s.items.length===2?"split":"")} ${clickAttr?"trainingClick":""}" ${clickAttr}><b>${s.start}–${s.end}</b><br>${esc(g.location)} · ${esc(g.baseName)}<br>${esc(labels)}${s.conflict?`<br><b class="bad">KONFLIKT</b>`:""}</div>`);
   }
...
async function loadCabins(){
 try{const j=await json("/api/v5/resources");resourceCache=j.items||[];}catch(e){console.error(e)}
}
function cabinResourceId(kind){
 const location=$("trLocation").value;
 const base=kind==="home"?"Heimkabine":"Gastkabine";
 return resourceCache.find(x=>x.location_id===location&&x.resource_type==="cabin"&&x.base_name===base)?.id||null;
}
$("trLocation").addEventListener("change",()=>{$("trCabin1").value="";$("trCabin2").value="";});
