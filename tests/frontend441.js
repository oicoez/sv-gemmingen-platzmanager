...
const $=id=>document.getElementById(id);let pin="",timer=null,includePast=false,weekStart="",monthValue="",serverToday="",editingTrainingId="",resourceCache=[],trainingMode="single";
...
document.addEventListener("click",async e=>{
 const target=e.target.closest("[data-training-id]");
 if(!target)return;
 const id=target.dataset.trainingId;
 if(!id)return;
...
$("login").onclick=async()=>{const p=prompt("Bearbeitungs-PIN:");if(!p)return;try{await json("/api/v5/auth/check",{method:"POST",headers:{"content-type":"application/json","x-edit-pin":p}});pin=p;$("sync").disabled=false;$("saveTraining").disabled=false;$("addTeam").disabled=false;$("login").textContent="Bearbeitungsmodus aktiv";loadTrainings()}catch(e){alert(e.message)}};
 ...
 try{
  ...
  for(const d of j.days){
   ...
   for(const g of d.groups)for(const s of g.segments){
    ...
    const onlyTraining=s.items.length===1&&s.items[0].eventType==="training"?s.items[0]:null;
    entries.push(`<div class="monthitem ${s.conflict?"conflict":(s.items.length===2?"split":"")} ${onlyTraining?"trainingClick":""}" ${onlyTraining?`data-training-id="${esc(onlyTraining.id)}" title="Training bearbeiten"`:""}><b>${s.start}–${s.end}</b><br>${esc(g.location)} · ${esc(g.baseName)}<br>${esc(labels)}${s.conflict?`<br><b class="bad">KONFLIKT</b>`:""}</div>`);
   }
