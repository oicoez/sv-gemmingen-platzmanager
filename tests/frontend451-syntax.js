...
const $=id=>document.getElementById(id);let pin="",timer=null,includePast=false,weekStart="",monthValue="",serverToday="",editingTrainingId="",resourceCache=[],trainingMode="single",editingGameId="",resourceOverviewCache=null;
...
async function loadTeams(){
 ...
 const j=await json("/api/v5/teams");
 $("trTeam").innerHTML=(j.items||[]).map(x=>`<option value="${esc(x.id)}">${esc(x.name)}</option>`).join("");
 if((j.items||[]).some(x=>x.id===selected))$("trTeam").value=selected;
 return j.items||[];
}
...
$("login").onclick=async()=>{const p=prompt("Bearbeitungs-PIN:");if(!p)return;try{await json("/api/v5/auth/check",{method:"POST",headers:{"content-type":"application/json","x-edit-pin":p}});pin=p;$("sync").disabled=false;$("saveTraining").disabled=false;$("addTeam").disabled=false;$("saveGame").disabled=false;$("resetGames").disabled=false;$("addPlace").disabled=false;$("login").textContent="Bearbeitungsmodus aktiv";loadTrainings()}catch(e){alert(e.message)}};
 ...
 try{
  ...
  for(const d of j.days){
   ...
   for(const g of d.groups)for(const s of g.segments){
    const labels=s.items.map(i=>`${i.eventType==="home_match"?"SPIEL":"Training"}: ${i.label}${i.sectionLabel!=="Gesamt"?` (${i.sectionLabel})`:""}`).join(" / "); const hasManual=s.items.some(i=>i.eventType==="home_match"&&i.manuallyChanged);
    const onlyItem=s.items.length===1?s.items[0]:null;
    const clickAttr=onlyItem?.eventType==="training"?`data-training-id="${esc(onlyItem.id)}" title="Training bearbeiten"`:(onlyItem?.eventType==="home_match"?`data-game-id="${esc(onlyItem.id)}" title="Spiel bearbeiten"`:"");
    entries.push(`<div class="monthitem ${s.conflict?"conflict":(s.items.length===2?"split":"")} ${clickAttr?"trainingClick":""}" style="--place-color:${esc(g.color||"#f5f7f9")}" ${clickAttr}><span class="placeTitle">${esc(g.location)} · ${esc(g.baseName)}</span><br><b>${s.start}–${s.end}</b><br>${esc(labels)}${hasManual?` <span class="manualBadge">MANUELL</span>`:""}${s.conflict?`<br><b class="bad">KONFLIKT</b>`:""}</div>`);
   }
...
function printRangeFromPreset(){
 ...
 if(preset==="month"){
  ...
  $("printFrom").value=`${y}-07-01`;$("printTo").value=`${y}-12-31`;
 }else if(preset==="rueckrunde"){
  const ry=m>=7?y+1:y;$("printFrom").value=`${ry}-01-01`;$("printTo").value=`${ry}-06-01`;
 }
}
$("printCalendar").onclick=()=>{$("printDialog").classList.remove("hidden");printRangeFromPreset()};
...
async function buildPrintMonth(ym,from,to){
 ...
 const [yy,mm]=ym.split("-").map(Number),firstDow=(new Date(Date.UTC(yy,mm-1,1)).getUTCDay()+6)%7,cells=[];
 for(let i=0;i<firstDow;i++)cells.push('<div class="pday blank"></div>');
 for(const d of j.days){
  if(d.date<from||d.date>to){cells.push(`<div class="pday mutedday"><b>${Number(d.date.slice(-2))}</b></div>`);continue}
  const entries=[];
  for(const g of d.groups)for(const s of g.segments){
   const labels=s.items.map(i=>`${i.eventType==="home_match"?"SPIEL":"Training"}: ${i.label}${i.sectionLabel!=="Gesamt"?` (${i.sectionLabel})`:""}`).join(" / ");
   const manual=s.items.some(i=>i.eventType==="home_match"&&i.manuallyChanged);
   entries.push(`<div class="pitem ${s.conflict?"conflict":""}" style="background:${printEsc(g.color||"#f5f7f9")}"><b>${printEsc(g.location)} · ${printEsc(g.baseName)}</b><br><strong>${s.start}–${s.end}</strong><br>${printEsc(labels)}${manual?' <span class="pmanual">MANUELL</span>':""}${s.conflict?'<br><b class="pred">KONFLIKT</b>':""}</div>`);
  }
  cells.push(`<div class="pday"><div class="pnum">${Number(d.date.slice(-2))}</div>${entries.join("")}</div>`);
 }
 return `<section class="printPage"><header><h1>ClubPlanner · Platzbelegung</h1><div>${printEsc(monthName(ym))}</div></header><div class="pgrid">${["Mo","Di","Mi","Do","Fr","Sa","So"].map(x=>`<div class="pdow">${x}</div>`).join("")}${cells.join("")}</div><footer>SV Gemmingen · erstellt ${new Date().toLocaleDateString("de-DE")}</footer></section>`;
}
$("startPrint").onclick=async()=>{
 ...
 try{
  ...
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>ClubPlanner_${from}_${to}</title><style>
   @page{size:A4 landscape;margin:7mm}*{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif;color:#14202b}
   .printPage{page-break-after:always;min-height:190mm;position:relative}.printPage:last-child{page-break-after:auto}
   header{display:flex;justify-content:space-between;align-items:baseline;border-bottom:2px solid #1f4e78;margin-bottom:4mm}h1{font-size:16pt;margin:0 0 2mm}header div{font-size:13pt;font-weight:700}
   .pgrid{display:grid;grid-template-columns:repeat(7,1fr);gap:1.4mm}.pdow{text-align:center;font-weight:700;font-size:8pt;padding:1mm}
   .pday{border:1px solid #bcc7d2;min-height:27mm;padding:1.2mm;font-size:6.8pt;overflow:hidden}.pday.blank,.mutedday{background:#f5f6f7}.pnum{font-weight:800;font-size:8pt;margin-bottom:.8mm}
   .pitem{border-left:2px solid #607d94;border-radius:2px;padding:1mm;margin:.7mm 0;line-height:1.25}.pitem.conflict{background:#ffeaea!important;border-left-color:#c62828}
   .pmanual{font-weight:800;border:1px solid #b68b00;border-radius:6px;padding:0 .8mm;background:#fff3cd}.pred{color:#b42318}
   footer{position:absolute;bottom:0;right:0;font-size:6.5pt;color:#66727e}@media print{button{display:none}}
