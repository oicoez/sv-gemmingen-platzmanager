 ...
 try{
  ...
  for(const d of j.days){
   ...
   for(const g of d.groups)for(const s of g.segments){
    ...
    entries.push(`<div class="monthitem ${s.conflict?"conflict":(s.items.length===2?"split":"")} ${clickAttr?"trainingClick":""}" style="--place-color:${esc(g.color||"#f5f7f9")}" ${clickAttr}><span class="placeTitle">${esc(g.location)} · ${esc(g.baseName)}</span><br><b>${s.start}–${s.end}</b><br>${esc(labels)}${s.conflict?`<br><b class="bad">KONFLIKT</b>`:""}</div>`);
   }
