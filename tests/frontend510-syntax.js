...
function setAdminMode(active){
 document.body.classList.toggle("adminMode",active);
 document.querySelectorAll(".adminOnly").forEach(x=>x.classList.toggle("hidden",!active));
 $("login").textContent=active?"Admin aktiv · Abmelden":"Admin";
 $("login").classList.toggle("primary",active);
 if(!active){
  document.querySelectorAll(".tab").forEach(x=>x.classList.toggle("active",x.dataset.view==="month"));
  ["dashboard","week","month","training","resources","teams","games"].forEach(v=>$(`view-${v}`).classList.toggle("hidden",v!=="month"));
  loadMonth();
 }
}
$("login").onclick=async()=>{
 if(pin){pin="";setAdminMode(false);return}
 const p=prompt("Admin-PIN:");if(!p)return;
 try{
  await json("/api/v5/auth/check",{method:"POST",headers:{"content-type":"application/json","x-edit-pin":p}});
  pin=p;$("sync").disabled=false;$("saveTraining").disabled=false;$("addTeam").disabled=false;$("saveGame").disabled=false;$("resetGames").disabled=false;$("addPlace").disabled=false;$("saveManual").disabled=false;
  setAdminMode(true);loadTrainings();
...
async function loadMonth(useServerCurrent=false){
 ...
 try{
  ...
  for(const d of j.days){
   ...
   cells.push(`<div class="monthday" data-date="${esc(d.date)}" title="${pin?"Klicken: manuellen Termin hinzufügen":"Nur lesen"}"><div class="monthnum"><span class="desktopDay">${Number(d.date.slice(-2))}</span><span class="mobileDate">${new Date(d.date+"T12:00:00").toLocaleDateString("de-DE",{weekday:"short",day:"2-digit",month:"2-digit"})}</span></div>${entries.join("")}</div>`);
  }
  ...
  document.querySelectorAll("#monthGrid .monthday[data-date]").forEach(el=>el.onclick=e=>{
   ...
   if(!pin)return;openManualDialog(el.dataset.date);
  });
...
window.addEventListener("beforeinstallprompt",e=>{
 ...
 if(!window.matchMedia("(display-mode: standalone)").matches)$("installApp").classList.remove("hidden");
});
$("installApp").onclick=async()=>{
 if(deferredInstallPrompt){
  deferredInstallPrompt.prompt();await deferredInstallPrompt.userChoice;
  deferredInstallPrompt=null;$("installApp").classList.add("hidden");return;
 }
 alert("iPhone/iPad: Im Browser auf Teilen → Zum Home-Bildschirm. Android: Browser-Menü → App installieren bzw. Zum Startbildschirm.");
};
window.addEventListener("appinstalled",()=>$("installApp").classList.add("hidden"));
if("serviceWorker" in navigator){window.addEventListener("load",()=>navigator.serviceWorker.register("/service-worker.js").catch(()=>{}))}
if(/iPhone|iPad|iPod/.test(navigator.userAgent)&&!window.navigator.standalone)$("installApp").classList.remove("hidden");
